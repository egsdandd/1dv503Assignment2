...
describe('cartController', () => {
  ...
  describe('viewCart', () => {
    ...
    test('should render cart with items and total', async () => {
      const mockRows = [
        { isbn: '111', qty: 2, title: 'Book 1', price: 10.5 },
        { isbn: '222', qty: 1, title: 'Book 2', price: 15.0 }
      ]
      mockDb.execute.mockResolvedValueOnce([mockRows])

      await viewCart(req, res, next)

      expect(mockDb.execute).toHaveBeenCalledWith(expect.stringContaining('SELECT c.isbn'), [1])
      expect(res.render).toHaveBeenCalledWith('cart/index', {
        items: [
          { isbn: '111', title: 'Book 1', price: 10.5, qty: 2, total: 21.0 },
          { isbn: '222', title: 'Book 2', price: 15.0, qty: 1, total: 15.0 }
        ],
        grandTotal: 36.0
      })
    })
  ...
  describe('checkout', () => {
    ...
    test('should create order and order details', async () => {
      ...
      expect(mockDb.execute).toHaveBeenCalledWith(expect.stringContaining('INSERT INTO odetails'), [
        100,
        '111',
        2,
        20
    ...
    test('should clear cart after checkout', async () => {
      ...
      expect(mockDb.execute).toHaveBeenCalledWith('DELETE FROM cart WHERE userid = ?', [1])
    })
    ...
    test('should render invoice with correct data', async () => {
      ...
      expect(res.render).toHaveBeenCalledWith('cart/invoice', {
        ...
        items: [{ isbn: '111', title: 'Book 1', price: 10, qty: 2, total: 20 }],
        grandTotal: 20
      })
    })
