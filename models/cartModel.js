...
export async function getCartItem(db, userId, isbn) {
  const [rows] = await db.execute('SELECT qty FROM cart WHERE userid = ? AND isbn = ?', [
    userId,
    isbn
  ])
  return rows[0] || null
...
export async function addItemToCart(db, userId, isbn, quantity) {
  await db.execute('INSERT INTO cart (userid, isbn, qty) VALUES (?, ?, ?)', [
    userId,
    isbn,
    quantity
  ])
...
export async function updateCartItemQuantity(db, userId, isbn, newQuantity) {
  await db.execute('UPDATE cart SET qty = ? WHERE userid = ? AND isbn = ?', [
    newQuantity,
    userId,
    isbn
  ])
